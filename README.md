...
TODO - Need a photo of the latest setup


## Intro

This repository is a complete build guide for the wireless Charybdis Mini (3x6), a split ergonomic rectilinear keyboard with integrated trackball. It covers the full wireless build process and pairs it with a purpose-built ZMK firmware repo designed for easy configuration and customization.
...
Huge thanks to Quentin and BastardKB for sharing this work with the community. If you prefer a kit or assembled keyboard, BastardKB sells both of these for the wired Charybdis, as well as a number of other keyboards and accessories at https://bastardkb.com/.
...
## Source & Purchase Parts - Bill of Materials
...
<p img align="left">
  <img src="images/pcbs/pcb-gerber-render-marketing.png" width="800">
</p>
...
While the [nice!nano v2.0](https://nicekeyboards.com/nice-nano/) is the standard controller board for wireless Charybdis builds, more budget-friendly options exist. For the most part, these are a drop-in replacement, but make sure you understand the tradeoffs before using one instead of the nice!nano.
...
> [!NOTE]
> The pin headers included with the controller board can be used in place of the socket headers, as long as they don't prevent the USB port from lining up with the case opening.


### DES Keycaps (optional)

<p img align="left">
  <img src="images/keycaps/des-keeb-keycaps.png" width="600">
</p>
...
Test the power connections before continuing. Avoid shorting any exposed connection while you plug in the JST battery connector, flip the switch on, and confirm the controller board powers on (an LED will light up). If all goes well, unplug the battery and continue assembly.
...
At this point, the keyboard assembly is complete. If you are using a standard dongle, put that in its case after flashing. If you're also building a Prospector dongle, continue with the build steps below.
...
| None | Brightness is fixed in firmware |
...
| M2.5 x 4   | Controller rear cap to body                         | 2        |
...
1. Flash the appropriate Prospector firmware onto the controller board. Use the [firmware build family table](#zmk-firmware) below to match your hardware, then follow the firmware repo links there for the flashing workflow.

2. Plug in the controller and confirm the display powers on and shows the Prospector theme you chose.

3. Fit the screen into the bezel and route the cable out the back through the bezel and body.

4. If using the non-touch screen, press the frame into place.

> [!NOTE]
> The frame is printed 2mm bigger than the ID of the bezel to create a tight friction fit.

5. Screw the body, bezel, and screen together.

6. If using an ambient light sensor, snap the sensor cover into place, then screw the sensor into the mounting ribs.

7. Slide the controller board into the controller rear cap. If using the nice!nano version, there are shoulders to lock it in, and the rear faces should prevent it from coming loose.
...
8. If using an ambient light sensor, put electrical tape over the controller board power LED so it does not interfere with the sensor.

9. Snap the controller rear cap into the body and screw the two pieces together.

10. Plug it in and confirm everything is still working.
